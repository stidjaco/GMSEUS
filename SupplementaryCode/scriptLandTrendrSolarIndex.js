...
/*
LandTrendr.js script with solar indices included 
Last edit date: 11-05-2024
Edited by: Jacob Stid | stidjaco@msu.edu
*/
  ...
  if('exclude' in exclude){
    ...
    if('slcOff' in exclude){
      ...
      if(exclude.slcOff === true){
        ...
        collection = collection.filter(ee.Filter.and(ee.Filter.eq('SPACECRAFT_ID', 'LANDSAT_7'),ee.Filter.gt('system:time_start', 1054425600000)).not()).aside(print, 'filteredCol');
...
// ###################################################################################################
// 
// Indices used to classify the presence of solar PV (NDPVI, NBD, Br, NDWI, and NDVI--already present)
//
// ###################################################################################################

// NDPVI
var ndpviTransform = function(img){ 
  var ndpvi = img.expression('(0.5*B1-B4)/(0.5*B1+B4)', {'B1': img.select('B1'), 'B4': img.select('B4')}) // no longer inverted from normal index (NDPVI = BLUE - NIR / BLUE + NIR ) -- not sure why we did this initially
                .multiply(1000) // scale results by 1000
                .select([0], ['NDPVI']) // name the band
                .set('system:time_start', img.get('system:time_start'));
  return ndpvi;
};

// NBD
var nbdTransform = function(img){ 
  var nbd = img.expression('(B1-((B3+B2)/2))/(B1+((B3+B2)/2))', {'B1': img.select('B1'), 'B2': img.select('B2'), 'B3': img.select('B3')}) 
                .multiply(1000) // scale results by 1000
                .select([0], ['NBD']) // name the band
                .set('system:time_start', img.get('system:time_start'));
  return nbd;
};

// Brightness
var brTransform = function(img){ 
  var br = img.expression('(B1+B2+B3+B4)/4', {'B1': img.select('B1'), 'B2': img.select('B2'), 'B3': img.select('B3'), 'B4': img.select('B3')}) 
                //.multiply(1000) // scale results by 1000
                .select([0], ['BR']) // name the band
                .set('system:time_start', img.get('system:time_start'));
  return br;
};
...
var calcIndex = function(img, index, flip){
  ...
  switch (index){
    ...
    case 'NDFI':
      ...
      break;
    // Solar indices -- NDPVI, NBD, BR, and NDWI, in theory would all increase with a loss in vegetation. So, don't flip. 
    case 'NDPVI':
      indexImg = ndpviTransform(img)//.multiply(indexFlip);
      break;
    case 'NBD':
      indexImg = nbdTransform(img)//.multiply(indexFlip);
      break;
    case 'BR':
      indexImg = brTransform(img)//.multiply(indexFlip);
      break;
    case 'NDWI':
      indexImg = ndwiTransform(img)//.multiply(indexFlip);
      break;
    default:
  ...
  */
  ...
  if(makeBoolean(changeParams.mmu.checked) === true){ 
    if(changeParams.mmu.value > 1){
      ...
      //var mmuMask = apply_mmu(mmulyr)           // MMU new method     
       var mmuMask = chngImg.select(['yod'])  // MMU old method 
